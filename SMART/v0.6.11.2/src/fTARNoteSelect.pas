...
uses
  ...
  Dialogs, StdCtrls, ORCtrls, ORDtTm, ORNet, ORFn, ComCtrls;

type
  TfrmATRNoteSelect = class(TForm)
...
var
  frmATRNoteSelect: TfrmATRNoteSelect;
...
procedure TfrmATRNoteSelect.rbtnNewNoteClick(Sender: TObject);
begin
  try
...
procedure TfrmATRNoteSelect.rbtnSelectAddendumClick(Sender: TObject);
begin
  try
...
procedure TfrmATRNoteSelect.rbtnSelectOwnClick(Sender: TObject);
begin
  try
...
procedure TfrmATRNoteSelect.buCancelClick(Sender: TObject);
{
...
procedure TfrmATRNoteSelect.DisplayTARAlertValues();
var
  i: integer;
...
procedure TfrmATRNoteSelect.FormShow(Sender: TObject);
var
  j: integer;
...
begin
  try
      ...
      begin
        ...
        FastAssign(frmTARNotification.RPCBroker.Results, slTIUNotes);
      end;

      //TFF - shouldn't the TIU Note IEN be the 8th piece of XQAID
      //      The 7th piece is the IEN of the VFD ALERT TRACKING file
      thisATRTIUNoteIEN := Piece(self.XQAID,';',8);
      //TFF - is this backwards?
      //TFF - changed <> '' to = ''
...
function TfrmATRNoteSelect.GetTIUNotesByDate(thisPatientDFN: string;
                           thisStartDate: TFMDateTime; //Start Date
...
procedure TfrmATRNoteSelect.lbATRNotesClick(Sender: TObject);
begin
  try
    rbtnSelectAddendum.Checked := true;
  except
