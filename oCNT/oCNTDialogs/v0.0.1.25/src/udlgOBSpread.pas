...
uses
  ...
  JvStringGrid, JvExControls, Vcl.Grids, JvExGrids, oCNTBase, uExtndComBroker;
...
begin
  ...
  cbxDispenceTime.Format := 'hh:mm tt';
  dtpicrExamDate.Format := 'MM/dd/yyyy';
end;
...
procedure TdlgOBSpread.BitBtn1Click(Sender: TObject);
var
  sl: TStringList;
begin
...
procedure TdlgOBSpread.btnCloseClick(Sender: TObject);
begin
...
begin
  SetValuesForDate(DateToStr(dtpicrExamDate.Date));
end;
...
procedure TdlgOBSpread.btnSaveClick(Sender: TObject);
var
    ...
    begin
      ...
      begin
        try
          ...
          begin
            if StrToDate(sgStandard.Cells[0, I]) < before then
            begin
              before := StrToDate(sgStandard.Cells[0, I]);
              beforeRow := I;
            end;
          end;
        except
...
begin
  ...
  begin
    ...
    if (btnEditMode.Caption = 'Enable Edit Mode') then
    sDate := DateToStr(Now - rdgrbxStandardExamDate.ItemIndex)
    else
    sDate := DateToStr(dtpicrExamDate.Date);
    ...
    sgStandardSelectCell(Sender, 1, NewRow, Select);
  end else
  begin
    if not ValidateSave then Exit;
    // Diabetic Page
    ...
    if (btnEditMode.Caption = 'Enable Edit Mode') then
    sDate := DateToStr(Now - rdgrbxDiabeticExamDate.ItemIndex)
    else
    sDate := DateToStr(dtpicrExamDate.Date);
    ...
    sgDiabeticSelectCell(Sender, 1, NewRow, Select);
  end;
  if not grbxExamDate.Visible then
  ClearInputValues;
end;
...
procedure TdlgOBSpread.GridSortColumn(Sender: TJvStringGrid; AColumn,
  ARow: Integer);
begin
  if not (Sender is TJvStringGrid) then Exit;
  if ARow <> 0 then Exit;
...
begin
  ...
  begin
    ...
    begin
      ...
      if (col_ptr <> -1) then
      begin
        cmbHeart.Text := sgStandard.Cells[col_ptr, row_ptr];
        cmbHeartChange(self);
      end;
      ...
      if (col_ptr <> -1) then
      cmbEdema.Text := sgStandard.Cells[col_ptr, row_ptr];

      col_ptr := sgStandard.Rows[0].IndexOf('Urine Glucose');
      if (col_ptr <> -1) then
      cmbSugar.Text := sgStandard.Cells[col_ptr, row_ptr];

      col_ptr := sgStandard.Rows[0].IndexOf('Urine Protein');
      if (col_ptr <> -1) then
      cmbProtein.Text := sgStandard.Cells[col_ptr, row_ptr];

      col_ptr := sgStandard.Rows[0].IndexOf('Fetal Activity');
      if (col_ptr <> -1) then
      cmbFetAct.Text := sgStandard.Cells[col_ptr, row_ptr];

      col_ptr := sgStandard.Rows[0].IndexOf('Presentation');
      if (col_ptr <> -1) then
      cmbPres.Text := sgStandard.Cells[col_ptr, row_ptr];
          ...
          begin
            ...
            begin
              ...
              begin
                ...
                begin
                  customVal.Visible := True;
                  customVal.Text := sgStandard.Cells[I, row_ptr];
                end;
                Break;
              end;
            end;
  ...
  begin
    ...
    begin
      ...
      if (col_ptr <> -1) then
      cbxDiet.Text := sgDiabetic.Cells[col_ptr, row_ptr];

      col_ptr := sgDiabetic.Rows[0].IndexOf('Control');
      if (col_ptr <> -1) then
      cbxControl.Text := sgDiabetic.Cells[col_ptr, row_ptr];
...
var
  Select: Boolean;
begin
  if (not grbxExamDate.Visible) or (sgDiabetic.Cells[0, ARow] = '') then Exit;

  dtpicrExamDate.DateTime := StrToDate(sgDiabetic.Cells[0, ARow]);
  btnEditClick(Sender);
  Select := True;
  if sgDrug.Cells[0,1] <> '' then
  sgDrugSelectCell(Sender, 0, 1, Select);
...
procedure TdlgOBSpread.sgStandardSelectCell(Sender: TObject; ACol,
  ARow: Integer; var CanSelect: Boolean);
begin
  if (not grbxExamDate.Visible) or (sgStandard.Cells[0, ARow] = '') then Exit;

  dtpicrExamDate.DateTime := StrToDate(sgStandard.Cells[0, ARow]);
  btnEditClick(Sender);
end;
