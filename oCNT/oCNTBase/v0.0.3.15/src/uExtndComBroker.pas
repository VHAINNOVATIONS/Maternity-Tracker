...
uses
  ...
  Trpcb, Hash, StdCtrls, CPRSChart_TLB, msxml;
...
type
  ...
  public
    ...
    { Common Functions }
    function SignOnVerify(Access,Verify: string): Boolean;
...
begin
  ...
  if I < PieceNum then
  Result := ''
  else SetString(Result, Strt, Last - Strt);
end;
...
function TCPRSComBroker.SignOnVerify(Access,Verify: string): Boolean;
var
  sl: TStringList;
begin
  Result := False;
  ...
  try
    try
      SetContext('XUS SIGNON');
      tCallV(sl, 'XUS AV CODE', [Encrypt(Access + ';' + Verify)]);
      if sl[0] <> '0' then Result := True;
    except
      on E: Exception do
...
function TCPRSComBroker.HasSecurityKey(const KeyName: string): Boolean;
...
begin
  ...
  try
    SetContext(MENU_CONTEXT);
    tCallV(Result, 'DSIO ORQQAL LIST', [PatientDFN]);
  except
    on E: Exception do
    begin
      Result.Clear;
...
begin
  ...
  try
    SetContext(MENU_CONTEXT);
    tCallV(Result, 'DSIO ORQQPL LIST', [PatientDFN]);
  except
    on E: Exception do
    begin
      Result.Clear;
...
begin
  ...
  try
    SetContext(MENU_CONTEXT);
    tCallV(Result, 'DSIO ORQQPS LIST', [PatientDFN]);
  except
    on E: Exception do
    begin
      Result.Clear;
...
begin
  ...
  try
    SetContext(MENU_CONTEXT);
    tCallV(Result, 'DSIO ORQQVI VITALS', [PatientDFN]);
  except
  end;
end;
