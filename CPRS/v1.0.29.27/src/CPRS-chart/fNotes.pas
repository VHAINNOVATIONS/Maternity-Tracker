...
uses
  ...
  VA508ImageListLabeler, RichEdit, rReminders;
...
type
  ...
  private
    ...
    FOrderID: string;
    FImageFlag: TBitmap;
//    FEditNote: TEditNoteRec;  //TFF - DSIO
...
//    property EditingIndex: Integer read FEditingIndex write SetEditingIndex;  //TFF - DSIO
...
begin
  ...
  try
    ...
    begin
      ...
      //TFF - deleting an ATR note and then creating a new note would create a note with
      // the details of the last note - the ATR note - bypassing user prompts because of the
      // following checks still in place
      if ((Notifications.Active) and (Notifications.FollowUp = NF_ATR)) then
        begin
          GetATRTitle(FEditNote,Piece(Notifications.RecordID,U,2)); //ATR
        end;
...
var
  ...
  Saved: boolean;
  slATRPtList: TStringList; //ATR
  i: integer; //ATR
  thisATRTIUNoteIEN: string; //ATR
  thisATRien: string; //ATR
begin
  inherited;
  if lstNotes.ItemIEN = 0 then Exit;
  if assigned(frmRemDlg) then
...
(*    with tvNotes do Selected := FindPieceNode(SavedDocID, U, Items.GetFirstNode);
  ...
  begin
    ...
    slATRPtList.Free;
    //TFF - added Notifications.Clear because processing a SMART alert would cause all other
...
begin
  ...
  if (Notifications.FollowUp <> NF_ATR) then  //ATR
    begin
  ...
  begin
    if bATRAval then //ATR
    begin
      LoadNotes;

      sTmp := ATRDocumentType(RPCBrokerV,Patient.DFN,IntTOStr(User.DUZ) , Piece(Notifications.RecordID,U,2)); //add XQAID
      sDocType := Piece(sTmp,U,1);
      //Added this condition 20120905, TAR.dll v0.6.1.0
      //to support cancelling out of TAR.dll Note Select dialog.
      if sDocType = '' then
      Exit;

      if sDocType = 'N' then
      cmdNewNoteClick(self)
      else
      if sDocType = 'A' then
      begin
        tvNotes.Select(tvNotes.FindPieceNode(Piece(sTmp,U,2),1)); //TTreeView.Select() //finds note based on IEN passed in
        mnuActAddendClick(self);
      end
      else
      if sDocType = 'O' then
      ShowMessage('Create Addendum to an Existing Note to document SMART');
