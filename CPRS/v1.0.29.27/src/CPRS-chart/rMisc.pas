...
function DisplayFilteredATRs(thisTime: string) : boolean;
procedure SaveShowFilteredATRs(sParam: string);
...
function DisplayFilteredATRs(thisTime: string): boolean;
begin
  if (Piece(thisTime,';',1) = '') then
    result := false
  else
    begin
    if (StrToTimeDef(thisTime, strToTime('23:59')) <= Time()) then
      result := true
    else
      result := false;
    end;
...
procedure SaveShowFilteredATRs(sParam: string);
begin
  CallV('ORATR SET FILTER PARAM',[sParam]);
end;
initialization
  // nothing for now
