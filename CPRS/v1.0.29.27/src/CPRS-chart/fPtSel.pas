...
type
  ...
  private
    ...
    procedure AlertList;
    procedure AlertListATROnly(List: TStringList);
...
var
  frmPtSel: TfrmPtSel;
  sTmp: string;                 //ATR
  sATRDisplayTime: string;      //ATR
  sATRDisplayTime_orig: string; //ATR
  isMainDlg: boolean;           //ATR
  isNagDlg: boolean;            //ATR
begin
  //ATR
  bNotATRPt := True;
  sATRDisplayTime := Piece(sATRFilterShowParam,';',1);
  sATRDisplayTime_orig := sATRDisplayTime;
  //ATR

  if bATRAval and (bATRPtSelOnce or frmFrame.mnuShowFilteredATRS.Checked or
                    frmFrame.mnuAlwaysShowFilteredATRs.Checked or
                    DisplayFilteredATRs(sATRDisplayTime)) then //ATR
    begin
     frmFrame.mnuShowFilteredATRS.Checked := false; //init
     slPtList := TStringList.Create;
     slPtList.Clear;
     bNotATRPt := GetATRPatient(slPtList, sATRDisplayTime, ORNet.RPCBrokerV, IntToStr(User.DUZ)); //TAR.dll - frmTARNotification.ShowModal
     //Not having this flag set to true when Count=0, causes CPRS to Exit below, rather than going to the Patient Select dialog
     if slPtList.Count = 0 then
      bNotATRPt := true;
     isMainDlg := true;
     isNagDlg := false;
     if sATRDisplayTime_orig <> sATRDisplayTime then
      begin
      sATRFilterShowParam := sATRDisplayTime + ';' + Piece(sATRFilterShowParam,';',2);
      SaveShowFilteredATRs(sATRFilterShowParam);
      end;
     bATRPtSelOnce := false;
    end;
  //ATR
  ...
  try
    ...
    begin
      ...
      Notifications.Clear;
      FsortCol := -1;
      if bNotATRPt then //ATR
       AlertList
      else
        begin
        AlertListATROnly(slPtList);
        //If there are no unprocessed TARs for this provider
        // then we want to skip the main TAR dialog, and
        // just go directly to the CRPS Patient Select dialog.
        CallV('ORATR PROVIDER',[User.DUZ]);
      ...
      FOKClicked := FALSE;
        try
          if (bNotATRPt = true) then  //ATR - Not ATR patient
            begin
             frmPtSel.ShowModal; //display the CPRS Patient Select dialog

             if bATRAval and frmPtSel.FOKClicked then
               begin
               bNotATRPt := false;
                slPtList.Clear;
                slPtList := PtProcessATRMult(RPCBrokerV, IntToStr(User.DUZ), Patient.DFN); //TAR.dll - frmTARProcess.ShowModal
                isMainDlg := false;
                isNagDlg := true;

                if (uATR.slPtList.Count > 0) then
                  begin
                  bNotATRPt := false;
                   lstvAlerts.Clear;
                   AlertListATROnly(slPtList);
                   cmdProcessClick(frmPtSel);
                  end
                ELSE
                  bNotATRPt := true;
               end
            ...
            on E: Exception do
              MessageDlg('A SMART exception has occurred in fPtSel.SelectPatient()' + CRLF + E.Message, mtError, [mbOk], 0);
          end;
      UserCancelled := FUserCancelled;
    if (not bNotATRPt) then //ATR - If it IS an ATR patient, then...
      if isNagDlg then
        begin
        if (slPtList.Count > 0) then
          cmdProcessAllClick(frmPtSel);
        end
      else
        cmdProcessAllClick(frmPtSel);
    end;
...
begin
  ...
  begin
    ...
    //   TIU6028;1423;3021203.09
      ...
      //ATR                                    //Process TAR ONLY if not marked "Completed"
      //eg. XQAID = ATR,101028,25;10000;101028;3120717.100508;1;3120726.1332;2^^1^^^2
      if (Copy(XQAID, 1, 3) = 'ATR') {and (strToInt(Piece(XQAID,'^',13)) < 2))} then
      //   ATR;NF-ATR;DFN
        begin
          x := XQAID;
          if Piece(x, ';', 3) <> '' then
            begin
              ADFN := Piece(x, ';', 3);  //*DFN*
              AFollowUp := StrToIntDef(Piece(x,';',2),0);
              Notifications.Add(ADFN, AFollowUp, RecordID + '^^' + Piece(x, U, 3));
              uATR.ATR_ien := Piece(XQAID,';',7);
              enableclose := true;
            end
          else
            DeleteAlert(XQAID);
...
procedure TfrmPtSel.AlertListATROnly(List: TStringList);
var
  NewItem: TListItem;
  I,J: Integer;
  Comment: String;
