...
procedure GetATRTitle(var NoteRec: TEditNoteRec; ATR_ID: string);
...
procedure GetATRTitle(var NoteRec: TEditNoteRec; ATR_ID: string);
begin
  CallV('ORATR GET NOTE',[Piece(ATR_ID,';',5)]);
  NoteRec.Title := strToInt(Piece(RPCBrokerV.Results[0],U,1));   //Note Pointer
  NoteRec.TitleName := Piece(RPCBrokerV.Results[0],U,2);         
end;
{ TIU Preferences  ------------------------------------------------------------------------- }
