...
uses
  ...
  VA508AccessibilityManager, fMHTest, fFrame, {//ATR} ORNet {//ATR};
...
uses fNotes, uPCE, uOrders, rOrders, uCore, rMisc, rReminders,
  ...
  VA508AccessibilityRouter, VAUtils, {//ATR} uATR {//ATR};
...
//  reText.Color := ReadOnlyColor;
 ...
 (* FSCRelated  := SCC_NA;
  ...
  if bATRAval then
    begin
    if (not bNotATRPt) then //It's a TAR patient
      begin
      cbTAR.Visible := true;
      CallV('ORATR SET TIU NOTE',[uATR.ATR_ien, RemForm.PCEObj.NoteIEN]);
      CallV('ORATR SET ATR PROCESSING LEVEL',[uATR.ATR_ien, 1]);
      CallV('ORATR SELECTED ALERT',[uATR.ATR_ien]);
      end
    else
      cbTAR.Visible := false;
    end;
...
begin
  ...
  //ATR
  //This note has been cancelled. So, remove the Note IEN from the associated
  //TAR Alert record that was written in TfrmRemDlg.FormCreate().
  if bATRAval then
    begin
    if (not bNotATRPt) then
      begin
  ...
  try
    ...
    try
      ...
      try
          ...
          if bATRAval then
            ...
            if (not bNotATRPt) then
              ...
              if cbTar.Checked then
                TmpText.Text := TmpText.Text + #13#10 + 'System for Mammogram Results Tracking (SMART) Alert is complete.' + #13#10#13#10;
              end;
            end;
          //ATR
          try
        ...
        //TFF - moved ATR block from the start to occur only if sccessful processing
        //ATR
        uATR.TARHealthFactors := ''; //ATR - init - See procedure TPCEData.SetHealthFactors()
          ...
          if bATRAval then
            ...
            if (bNotATRPt = false) then    //<=================== ORIG
              ...
              if not cbTAR.Checked then
                begin
                //MessageDlg('An exception has occurred in GetATRPatient()', mtError, [mbOk], 0);
                TARResponse := MessageDlg('The checkbox, ''System for Mammogram Results Tracking (SMART) Alert is complete'' is not checked.' + #13#10 +
                                          'Click ''Yes'' to mark this alert as complete.  Otherwise, click ''No''', mtConfirmation, [mbYes,mbNo], 0);
            ...
            if (bNotATRPt = false) then
              ...
              if cbTar.Checked then
                begin
                CallV('ORATR SET TIU NOTE',[uATR.ATR_ien, RemForm.PCEObj.NoteIEN]);
                CallV('ORATR SET ATR PROCESSING LEVEL',[uATR.ATR_ien, 2]); //Complete - Ready for transmit
                end
              else
                CallV('ORATR SET ATR PROCESSING LEVEL',[uATR.ATR_ien, 1]); //Incomplete - Some action taken
              end;
            end;
        //ATR
        ...
        try
          if sCallV('DSIO IS IT CONTROLLED?', [Rem.IEN + ';PXRMD(801.41,']) = '1' then
          begin
    ...
    try
      if uATR.slPtList.Count > 0 then
        if uATR.TARHealthFactors <> '' then
          CallV('ORATR HEALTH FACTORS',[uATR.ATR_ien, uATR.TARHealthFactors]);
    except
      on E: Exception do
        MessageDlg('A SMART exception has occurred in frmRemDlg.btnFinishClick()' + CRLF + E.Message, mtError, [mbOk], 0);
    end;
    //ATR
