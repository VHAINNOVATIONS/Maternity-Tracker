...
type
  TfrmFrame = class(TfrmBase508Form)
    ...
    Z9: TMenuItem;
    mnuShowFilteredATRS: TMenuItem;
    mnuAlwaysShowFilteredATRs: TMenuItem;
    ...
    procedure tabPageMouseDown(Sender: TObject; Button: TMouseButton;
      Shift: TShiftState; X, Y: Integer);
    procedure mnuShowFilteredATRSClick(Sender: TObject);
    procedure mnuAlwaysShowFilteredATRsClick(Sender: TObject);
...
uses
  ...
  fPrintLocation, fTemplateEditor, fTemplateDialog, fCombatVet, uATR;
...
{$ENDIF}
  ...
  if bATRAval then
  begin
   mnuAlwaysShowFilteredATRs.Enabled := true;
   mnuShowFilteredATRs.Enabled := true;
   mnuAlwaysShowFilteredATRs.Visible := true;
   mnuShowFilteredATRs.Visible := true;
   Z8.Visible := true;
   sATRFilterShowParam  := GetUserParam('ORATR FILTER TAR');
   if (Piece(sATRFilterShowParam,';',2) = '1') then
    mnuAlwaysShowFilteredATRs.Checked := true;
  end; //ATR
...
begin
  ...
  begin
    ...
    if ((SaveDFN <> NewDFN) or (Notifications.FollowUp = NF_ATR)) then  //ATR
    begin
...
(*          // hide the 'next notification' button
    ...
    NF_ATR                           : NextIndex := PageIDToTab(CT_NOTES); //ATR
    else InfoBox(TX_UNK_NOTIF, TC_UNK_NOTIF, MB_OK);
    end;
    tabPage.TabIndex := NextIndex;
    tabPageChange(tabPage);
    //ATR
    if Notifications.FollowUp = NF_ATR then
      begin
      if bATRAval then
        uATR.ATR_ien := Piece(Piece(Notifications.RecordID,'^',2),';',7);
      end;
    //ATR
  end
...
procedure TfrmFrame.mnuShowFilteredATRSClick(Sender: TObject);
begin
  inherited;
  mnuShowFilteredATRS.Checked := not(mnuShowFilteredATRS.Checked);
end;

procedure TfrmFrame.mnuAlwaysShowFilteredATRsClick(Sender: TObject);
begin
  inherited;
 mnuAlwaysShowFilteredATRS.Checked := not(mnuAlwaysShowFilteredATRS.Checked);
 if mnuAlwaysShowFilteredATRS.Checked then
 begin
  sATRFilterShowParam := Piece(sATRFilterShowParam,';',1) + ';1';
 end
 else
 begin
  sATRFilterShowParam := Piece(sATRFilterShowParam,';',1) + ';0';
 end;
