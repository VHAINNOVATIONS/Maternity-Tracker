...
//  DSIOstr,DSIOseq: Array of string; // TFF - DSIO
...
begin
  ...
  begin
    ...
    begin
      ...
      SetPiece(Tmp,U,20,Piece(Tmp,U,3)); // TFF - DSIO use this or DSIO(str,seq)
      SetPiece(Tmp,U,3,FTaxID);
//      If adding the Reminder Dialog IEN (FTaxID) is too invasive use this
//      if Piece(TempSL[i],U,3) <> '' then
//      begin
//        SetLength(DSIOstr, Length(DSIOstr)+1);
//        SetLength(DSIOseq, Length(DSIOseq)+1);
//        DSIOstr[Length(DSIOstr)-1] := Tmp;
//        DSIOseq[Length(DSIOseq)-1] := Piece(TempSL[i],U,3);
//      end;
      TempSL[i] := Tmp;
    end;
  ...
  begin
    ...
    try
      ...
      begin
        ...
        Result.Add('S^' + Fld + TCPRSDialogParentCheckBox(Sender).Caption + U + Seg)
      ...
      begin
        ...
        Result.Add('S^' + Fld + TCPRSDialogCheckBox(Sender).Caption + U + Seg)
      ...
      else if Sender is TCPRSNumberField then
      begin
        Result.Add('S^' + Fld + TCPRSNumberField(Sender).Text + U + Seg);
      end
      ...
      begin
        ...
        Result.Add('WP^' + Fld + TCPRSDialogRichEdit(Sender).Lines.Strings[x]);

        Result.Add('WP^' + Fld + TCPRSDialogRichEdit(Sender).Lines.Strings[x] + U + Seg);
      end
...
//  If adding the Reminder Dialog IEN (FTaxID) is too invasive use this
//  function GetSubIEN(str: string): string;
//  var
//    I: Integer;
//  begin
//    Result := '-1';
//    for I := 0 to Length(DSIOstr) - 1 do
//    begin
//      if DSIOstr[I] = str then
//      Result := DSIOseq[I];
//    end;
//  end;
...
begin
  ...
  Fld := TWinControl(AObject.FCurrentControl).ClassName + '|' +
         Piece(AObject.FRec4,U,2) + ',' + Piece(AObject.FRec4,U,20){GetSubIEN(AObject.FRec4)}
         + U + Piece(AObject.FRec4,U,8) + U;
...
begin
  ...
  try
    ...
    begin
      try
        CallV('DSIO OCNT STORE', [NoteIEN, Note, 'R' + Rem.GetIEN]);
      except
      end;
    end;
  finally
...
initialization
  InitReminderObjects;
//  SetLength(DSIOstr, 0);   // TFF - DSIO
//  SetLength(DSIOseq, 0);   // TFF - DSIO

finalization
  FreeReminderObjects;
//  DSIOstr := nil;          // TFF - DSIO
//  DSIOseq := nil;          // TFF - DSIO
