...
function GetLinkedTemplateData(const Link: string): string;
begin
  if ((Notifications.Active) and (Notifications.FollowUp = NF_ATR)) then         //ATR - TFF
  Result := GetATRTemplate
  else
...
begin
  ...
  begin
    ...
    //Result - 2049^T^A^ATR BI-RADS FOLLOW-UP^0^^^0^0^0^0^0^0^0^0^339^DSSVET-ATR BI-RAD SELECTOR^0^^^22;TIU(8925.1,^^^^0
