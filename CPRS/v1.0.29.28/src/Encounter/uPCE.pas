...
uses Windows, SysUtils, Classes, ORFn, uConst, ORCtrls, ORClasses,UBAGlobals, {//ATR}uATR, Dialogs{//ATR};
...
begin
  ...
  begin
    ...
    //ATR
    try
    //initialized in procedure TfrmRemDlg.btnFinishClick()
    uATR.TARHealthFactors := uATR.TARHealthFactors + TPCEHealth(CurHealthFactor).ItemStr;
    except
      on E: Exception do
      MessageDlg('A SMART exception has occurred in PCEData.SetHealthFactors()' + CRLF + E.Message, mtError, [mbOk], 0);
    end;
    //ATR
  end; {for}
end;
