...
begin
  ...
  try
    try
      sl.Text := DisplayDialog(@RPCBrokerV, Note.Configuration[TCheckListBox(Sender).ItemIndex], False);
    except
    end;
...
begin
  ...
  try
    try
      sl.Text := DisplayDialog(@RPCBrokerV, Note.Configuration[TListBox(Sender).ItemIndex], False);
    except
    end;
...
begin
  ...
  begin
    ButtonWidth := ((ClientWidth - (ControlCount - 1)) div (ControlCount - 1)) - 4;
    ...
    begin
      ...
      begin
        ButtonLeft := 1;
        Controls[I].SetBounds(ButtonLeft, ButtonTop, 25, ButtonHeight);
      end else
      begin
...
begin
  DDCSAbout := TDDCSAbout.Create(Owner);
  try
    DDCSAbout.ShowModal;
  finally
    DDCSAbout.Free;
...
begin
  if MessageDlgPos('Are you sure you want to exit?',mtconfirmation,[mbYes,mbNo],0,
                  (TForm(Owner).Left + (TForm(Owner).Width div 2) - 120),
...
begin
  ReviewNoteDlg := TReviewNoteDlg.Create(Owner);
  try
...
var
  ...
  procedure FindAndSetFocus(Control: TWinControl);
  var
    vControl: TWinControl;
    iPageControl: TPageControl;
    iPage,iTab: Integer;
  begin
    ShowDialog(Owner, VALID_MESS, mtWarning);
    Validated := False;
    ...
    repeat
      begin
        ...
        begin
          if vControl.ClassType = ToPage then
            iPage := ToPage(vControl).PageIndex;

          if vControl.ClassType = TTabSheet then
          begin
            iPageControl := TPageControl(TTabSheet(vControl).Parent);
            iTab := TTabSheet(vControl).PageIndex;
          end;
        end;
      end;
    until ((vControl = nil) or (vControl.ClassType = ToForm));

    if iPage <> -1 then
      ActivePageIndex := iPage;
    if iTab <> -1 then
      iPageControl.ActivePageIndex := iTab;
    Control.SetFocus;
  end;

  function CheckListForCheck(cb: TCheckListBox): Boolean;
  var
    I: Integer;
  begin
    Result := False;
...
begin
  ...
  try
    ...
    begin
      if DateToStr(TDateTimePicker(Sender).DateTime) <> '' then
      begin
        if not Hide then
        begin
          if Title <> '' then
    ...
    begin
      if CheckListForCheck(TCheckListBox(Sender)) then
      begin
        if not Hide then
        begin
          if Title <> '' then
    ...
    begin
      if Trim(TSpinEdit(Sender).Text) <> '' then
      begin
        if not Hide then
        begin
          if Title <> '' then
        ...
        begin
          ...
          begin
            ...
            begin
              Build.Clear;
              try
                Build := BuildDiscreetData(Item.OwningObject,Item.Suffix);
