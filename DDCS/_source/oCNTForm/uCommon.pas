...
type
  ...
  function BuildDiscreetData(Sender: TWinControl): TStringList;
...
begin
  ...
  if Sender.ClassType = TDateTimePicker then
    Result.Add(TDateTimePicker(Sender).Name + '^^' + DateToStr(TDateTimePicker(Sender).DateTime))
  else if Sender.ClassType = TEdit then
    Result.Add(TEdit(Sender).Name + '^^' + TEdit(Sender).Text)
  else if Sender.ClassType = TLabeledEdit then
    Result.Add(TLabeledEdit(Sender).Name + '^^' + TLabeledEdit(Sender).Text)
  else if Sender.ClassType = TCheckBox then
  begin
    if TCheckBox(Sender).Checked then
    begin
      if TCheckBox(Sender).Caption <> '' then
        Result.Add(TCheckBox(Sender).Name + '^^' + TCheckBox(Sender).Caption)
      else
        Result.Add(TCheckBox(Sender).Name + '^^' + TCheckBox(Sender).Hint);
  ...
  begin
    ...
    begin
      if TRadioButton(Sender).Caption <> '' then
        Result.Add(TRadioButton(Sender).Name + '^^' + TRadioButton(Sender).Caption)
      else
        Result.Add(TRadioButton(Sender).Name + '^^' + TRadioButton(Sender).Hint);
    end
  ...
  begin
    ...
    Result.Add(TSpinEdit(Sender).Name + '^^' + TSpinEdit(Sender).Text)
  else if Sender.ClassType = TComboBox then
  begin
    for I := 0 to TComboBox(Sender).Items.Count - 1 do
    begin
      if TComboBox(Sender).Items.Strings[I] = TComboBox(Sender).Text then
        Result.Add(TComboBox(Sender).Name + U + IntToStr(I) + 'TRUE^' + TComboBox(Sender).Items.Strings[I])
      else
        Result.Add(TComboBox(Sender).Name + U + IntToStr(I) + U + TComboBox(Sender).Items.Strings[I]);
    end;
  end else if Sender.ClassType = TListBox then
  begin
    ...
    begin
      ...
      begin
        if TListView(Sender).Items[I].Checked then
          Result.Add(TListView(Sender).Name + '^TRUE^' + tmpstr)
        else
          Result.Add(TListView(Sender).Name + '^FALSE^' + tmpstr);
      end else
        Result.Add(TListView(Sender).Name + '^^' + tmpstr);
    end;
...
begin
  ...
  try
    ...
    if ((Result = 0) and ((DDCSConfig <> '') and (StrToBool(DDCSConfig)))) then
      ShowMessage(Concat('This interface requires DDCSDialogs.dll to be present.',#10#13,'This interface may not work correctly.'));
  except
    on E: Exception do
