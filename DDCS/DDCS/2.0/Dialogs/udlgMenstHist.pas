...
begin
  ...
  begin
    ...
    if edtFrequency.Text  <> '' then TmpStrList.Add('  Frequency: ' + edtFrequency.Text + ' days');
    ...
    begin
      TmpStrList.Add('  What type(s) of contraception were you using most recently?');
      TmpStrList.Add('  '+ edtContraceptionType.Text);
    end;

    if edthcg.Text  <> '' then
      TmpStrList.Add('  hCG+: ' + edthcg.Text);
...
begin
  ...
  begin
    ...
    begin
      ...
      if DateToStr(TdlgEstDelivDate(TTabSheet(Parent).PageControl.Pages[I].Controls[J]).dtLMP.Date) <> edtLMP.Text then
      begin
